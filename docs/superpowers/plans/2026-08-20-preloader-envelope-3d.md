...
export function createEnvelopeScene({ coverTexture = null } = {}) {
  ...
  const frontMaterial = new THREE.MeshStandardMaterial({ color: 0xffffff, map: coverTexture, roughness: 0.45, metalness: 0.04 });
...
Expected: eleven tests pass: four stage-helper tests and seven envelope-scene tests.
...
Expected: the contract command prints `GREEN: preloader canvas boundary confirmed`; eleven unit tests pass; Vite exits with code `0`.
...
Expected: the contract command prints `GREEN: React preloader integration confirmed`; eleven unit tests pass; Vite exits with code `0`.
...
Expected: eleven tests pass, Vite writes `dist/index.html`, and `git diff --check` prints no errors.
...
- **Preloader layout and short viewport handling (`preloader.css`)**: The fixed full-screen preloader uses `inset: 0` and `overflow: hidden`; header, progress, hint, CTA, and fallback elements use safe-area-aware offsets, padding, and max-width constraints based on `env(safe-area-inset-*)` to stay within the viewport. There is no `overflow-wrap: anywhere` declaration. A `@media (max-height: 600px)` breakpoint tightens spacing, panel sizing, and CTA positioning for short viewports.
